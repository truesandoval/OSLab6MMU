...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  if(!avail_block_node){
    ...
    if(!prev){
      ...
      prev->next = avail_block_node->next;
    }
  }
  int blk_end = avail_block_node->blk->end;
  avail_block_node->blk->pid = pid;
  avail_block_node->blk->end = avail_block_node->blk->start + blocksize - 1;
  list_add_ascending_by_address(alloclist, avail_block_node->blk);
  if (avail_block_node->blk->end != blk_end){
    block_t *fragment = (block_t*)malloc(sizeof(block_t));
    
    fragment->pid = 0;  //set fragment pid to 0
    fragment->start = avail_block_node->blk->end + 1;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  ...
  if (!current){
    printf("Error: Unable to locate memory used by PID: %d\n", pid);
    return;
  }
  if (!prev){
